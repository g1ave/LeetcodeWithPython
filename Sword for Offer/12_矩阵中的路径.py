...
def out(x: int, y: int, m: int, n: int):
    if 0 <= x < m and 0 <= y < n:
        return False
    return True
...
class Solution:
    def exist(self, board: List[List[str]], word: str) -> bool:
        ...
        def dfs(x, y, index):
            if out(x, y, m, n) or board[x][y] != word[index]:
                return False
            if index == len(word) - 1:
                return True
            board[x][y] = ''
            res = dfs(x + 1, y, index + 1) or dfs(x - 1, y, index + 1) or dfs(x, y + 1, index + 1) or dfs(x, y - 1, index + 1)
...
if __name__ == '__main__':
    ...
    print(ans)
    s2 = Solution()
    assert s.exist([["C","A","A"],["A","A","A"],["B","C","D"]], "AAB")
